...
function actualizarUIsegunSesion() {
  if (sesion.token) {
    ...
    if (avisoLoginComentario) avisoLoginComentario.style.display = 'none';
  } else {
    if (formComentario) formComentario.style.display = 'none';
    if (avisoLoginComentario) avisoLoginComentario.style.display = 'block';
  }
}
...
if (!contenedorPost || !listaComentarios || !formComentario) {
  ...
  formComentario.addEventListener('submit', async (e) => {
    ...
    if (!sesion.token) {
      mostrarMensaje('mensaje-comentario', 'Debes iniciar sesión o registrarte para comentar.', 'error');
      return;
    }
    ...
    try {
      ...
      if (respuesta.status === 401 || respuesta.status === 403) {
        mostrarMensaje('mensaje-comentario', 'Tu sesión no es válida. Inicia sesión nuevamente.', 'error');
        return;
      }
...
function inicializarEventosReporte() {
  ...
  btnConfirmar.addEventListener('click', async () => {
    ...
    if (!sesion.token) {
      mostrarMensaje('mensaje-reporte', 'Debes iniciar sesión para reportar una publicación.', 'error');
      return;
    }
    ...
    try {
      ...
      if (respuesta.status === 401 || respuesta.status === 403) {
        mostrarMensaje('mensaje-reporte', 'Tu sesión no es válida. Inicia sesión nuevamente.', 'error');
        return;
      }
